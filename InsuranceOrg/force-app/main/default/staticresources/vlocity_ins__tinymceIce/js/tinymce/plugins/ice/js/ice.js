...
//						range.setStart(range.endContainer.parentNode.nextSibling, 0);
    ...
    _currentUserIceNode: function (node) {
      ...
      if (ice.dom.attr(node, this.userIdAttribute) == this.currentUser.id) {
          if (node && node.lastElementChild && node.lastElementChild.nodeName === 'BR') {
            return false;
          } else {
              return true;
          }
    ...
    _deleteSelection: function (range) {
      ...
      var bookmark = new ice.Bookmark(this.env, range),
        ...
        if (elements.length > 0) {
            var tmpEl = ice.dom.parents(elements[0], 'td')[0];
            if (tmpEl && tmpEl !== null && tmpEl.nodeName === 'TD' && tmpEl.className.indexOf('mce-item-selected') !== -1) {
                var parentTR = tmpEl.parentElement;
                siblingTR = parentTR.nextSibling;
                while (siblingTR !== null) {
                    if (siblingTR.children !== undefined) {
                        for (var i = 0; i < siblingTR.children.length; i++) {
                            var childTD = siblingTR.children[i];
                            if (childTD.nodeName === 'TD' && childTD.className.indexOf('mce-item-selected') !== -1) {
                                if (childTD.childNodes !== undefined) {
                                    elements.push(childTD.childNodes[0]);
                                }
                            }
                        }
                    }
    ...
    _initializeRangeLibrary: function () {
      ...
      rangy.rangePrototype.getNextContainer = function (container, skippedBlockElem) {
        ...
        while (container && !container.nextSibling) {
            if (container.parentNode.nodeName === 'TD') {
                container.parentNode.appendChild(this.env.document.createTextNode(''));
            } else if (container.parentNode.nextSibling && container.parentNode.nextSibling.nodeName === 'BR') {
                container.parentNode.insertBefore(this.env.document.createTextNode(''), container.parentNode.nextSibling);
            } else {
...
IceAddTitlePlugin.prototype = {
  nodeCreated: function(node, option) {
    node.setAttribute('data-title', (option.action || 'Modified') + ' by ' + node.getAttribute(this._ice.userNameAttribute)
...
IceCopyPastePlugin.prototype = {
  ...
  keyDown: function(e) {
    if(e.metaKey)
    {
        console.log('IceCopyPastePlugin:keyDown: ' + e.keyCode);
    }
