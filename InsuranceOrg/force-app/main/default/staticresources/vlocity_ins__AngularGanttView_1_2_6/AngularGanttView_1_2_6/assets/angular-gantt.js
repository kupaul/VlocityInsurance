    ...
    angular.module('gantt').factory('GanttCalendar', ['$filter', 'moment', function($filter, moment) {
        ...
        DateFrame.prototype.dateMatch = function(date) {
            if (this.evaluator) {
                ...
                //return date >= this.start && date <= this.end; //vineeta replaced the current line with the one below 
...
(function(){
    ...
    angular.module('gantt').factory('GanttCurrentDateManager', ['moment', function(moment) {
        ...
        //vineeta s
        GanttCurrentDateManager.prototype.showHolidaysAsLines = function(holidayDateFrames){
            console.log(holidayDateFrames);
            var i=0;
            var key = 'partialHoliday_'+i;
            var keyPos = 'pos_'+key;
            this['partialHolidays'] = [];
            do{
                console.log(holidayDateFrames[key]);
                var newElement, oldElement;
                if(holidayDateFrames[key].start!=undefined)
                {
                    var column = this.gantt.columnsManager.getColumnByDate(holidayDateFrames[key].start, true);
                    if (column !== undefined && column.$element !== undefined) {
                        newElement = column.$element;
                    }
                    this[key] = newElement; //needed?
                    var phPosition = this.gantt.getPositionByDate(holidayDateFrames[key].start, true); 
                    this['partialHolidays'].push(phPosition);
                    //debugger;
                    if(newElement !== undefined){
                        //newElement.addClass('gantt-foreground-col-current-date');
                        //newElement.addClass('gantt-current-date-line');
                    }
                }
                //debugger;
                i++;
                key = 'partialHoliday_'+i;
                var keyPos = 'pos_'+key;
            }while(holidayDateFrames[key]!=null);
...
(function(){
    ...
    angular.module('gantt').factory('GanttColumnHeader', [ 'moment', 'GanttColumn', function(moment, Column) {
        ...
        var ColumnHeader = function(date, viewScaleValue, viewScaleUnit, left, width, labelFormat) {
            ...
            if(viewScaleUnit === "week" || viewScaleUnit == "week"){
                //var monthYearWeekLabel = angular.isFunction("MMM") ? labelFormat(column): startDate.format("MMM");
                column.label = "Week "+column.label; 
            }
            //118e
            return column;
        };
...
(function(){
    ...
    angular.module('gantt').factory('GanttColumnsManager', ['GanttColumnGenerator', 'GanttHeaderGenerator', '$filter', '$timeout', 'ganttLayout', 'ganttBinarySearch', 'moment', function(ColumnGenerator, HeaderGenerator, $filter, $timeout, layout, bs, moment) {
        ...
        ColumnsManager.prototype.updateColumnsMeta = function() {
            ...
            var holidayDateFrames = this.gantt.options.value('dateFrames');
            //debugger;
            //this.gantt.currentDateManager.setCurrentDate(currentDateValue);
            if(holidayDateFrames!=null && holidayDateFrames.length!=0 && holidayDateFrames['partialHoliday_0']!=null){
                //debugger;
                this.gantt.currentDateManager.showHolidaysAsLines(holidayDateFrames);
            }
...
angular.module('gantt.templates', []).run(['$templateCache', function($templateCache) {
    $templateCache.put('template/gantt.tmpl.html',
        ...
        '<div ng-show="(viewScale==\'hour\'||viewScale==\'day\' || viewScale==\'week\' ||viewScale==\'month\'||viewScale==\'year\')&& gantt.currentDateManager.partialHolidays.length >= 0" ng-repeat="ph in gantt.currentDateManager.partialHolidays track by $index" class="gantt-current-date-line" style="border:3px solid red" class="gantt-current-date-line" ng-show="ph >= 0 && ph <= gantt.width" ng-style="{\'left\': ph + \'px\' }"></div>\n' +
        '            </gantt-body-foreground>\n' +
        '            <gantt-body-columns>\n' +
