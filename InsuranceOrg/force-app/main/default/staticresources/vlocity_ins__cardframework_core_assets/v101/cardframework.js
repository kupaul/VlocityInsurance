    ...
    _$provide.decorator('performAction', function($delegate) {
        ...
        var wrapper = function (action, actionConfig, obj, data, filter, pickedState) {
                if ('parentIFrame' in window) {
                    ...
                    action.contextId = record ? record.Id : null;
...
.factory('dataService', function($rootScope, remoteActions, force, nameSpaceService, debugService, pageService, $interpolate, $log, $q, $httpParamSerializer, cometd) {
            ...
            return nameSpaceService.getNameSpacePrefix().then(
                function(nsPrefix) {
                    ...
                    if (typeof remoteActions != 'undefined' && remoteActions != null && remoteActions['getDataViaDataRaptor']) {
                        ...
                        return myService.getApexRest(apexRestUrl,'GET', null, forcetkClient).then(
                            function(records) {
...
.factory('dataSourceService',function(dataService, $interpolate, $localizable, $q, $http, $log, $timeout, $rootScope, userProfileService, $filter) {
    ...
    function handleDataRaptor(datasource, scope, deferred, errorContainer, forcetkClient) {
        ...
        // To support older datasource definition
        if (datasource.value.ctxId) {
        ...
        dataService.getDataRaptorBundle(cardBundle, '', forcetkClient, inputMap).then(
            function(records) {
                $log.debug(records);
                var hasError = true;
                var errorMsg;
    ...
    selectResultNode: function(datasource, records, nodeVar) {
        ...
        if(datasource.orderBy &&  datasource.orderBy.name) {
...
angular.module('CardFramework')
    ...
    return {
        ...
        controller: ['$scope', '$rootScope','$element','$q','$controller','$attrs','timeStampInSeconds','actionService', 'dataService', '$interpolate','pageService','networkService','configService','performAction','$filter','$http', 'dataSourceService', '$vlocFlyout', '$log','interactionTracking', '$timeout',
         function($scope, $rootScope, $element, $q, $controller, $attrs, timeStampInSeconds, actionService, dataService,
              $interpolate, pageService, networkService, configService, performAction, $filter, $http, dataSourceService, $vlocFlyout, $log, interactionTracking, $timeout) {
                    ...
                    if ($scope.sObjects.length > 0) {
                        ...
                        if (!$scope.data.obj || !$scope.isRedraw) {
                            angular.forEach($scope.sObjects,function(sObject, i) {
                                ...
                                cardData.uniqueId = cardData.cardName + '--' + cardData.cardIndex; 
                            ...
                            if(cardDataArray.length > 0) {
                                $timeout(function() {
                                    ...
                                    $scope.$emit($scope.data.layoutName+'.addCard', cardDataArray);
                                    $scope.sObjects = []; 
                                });
                            }    
...
angular.module('CardFramework')
    ...
    return {
        ...
        function($scope, $rootScope, $element, $window, $log, $controller, $filter, $attrs, $http, force, configService, dataService,
            ...
            unbindEvents[unbindEvents.length] =
                $scope.$on($scope.uniqueName + '.addCard', function(event, cards) {
                ...
                if (!ignoreFields || ignoreFields.indexOf(field) === -1){
                    if (typeof sObject[field] === 'object') {
                        ...
                        try {
                            ...
                            if(filterObject[field].toLowerCase() === 'null' &&  _.get(sObject, field) === null) {
                                isFieldMatch = true;
                            }
                        } catch(err) {
                        ...
                        function(data) {
                            if (data.length > 0) {
                                ...
                                if (cardNames.length > 0) {
                                    ...
                                    return remoteActions.getActiveCardsByNames(cardNames).then(
                                            function(data) {
                                                var cards = data;
                                                //pre-filling array with nulls to make sure cards get added to the right index in the Deck array
                        ...
                        function(data) {
                            ...
                            if (data.records.length > 0) {
                                ...
                                if (cardNames.length > 0) {
                                    ...
                                    var cardsSoql = 'SELECT Id, Name, ' + nsPrefix + 'Type__c, ' + nsPrefix + 'Definition__c, LastModifiedDate, ' + nsPrefix + 'Version__c, ' + nsPrefix + 'Active__c FROM ' + nsPrefix + 'VlocityCard__c WHERE  ' + nsPrefix + 'Active__c = true AND Name IN ';
