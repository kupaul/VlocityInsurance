...
.config(['$localizableProvider','dataServiceProvider', function($localizableProvider,dataServiceProvider) {
    ...
    var map = {}, 
    asyncMode = true;
    $localizableProvider.$get = function $LocalizableFactory(remoteActions, $rootScope, $timeout, $q) {
...
.factory('dataSourceService',function(dataService, $interpolate, $localizable, $q, $http, $log, $timeout, $rootScope, userProfileService, $filter, interactionTracking) {
    ...
    function handleApexRemote(datasource, scope, deferred, errorContainer) {
        ...
        var asyncDataMap = {isAsync : datasource.value.vlocityAsync || false, asnycTimeout : datasource.value.vlocityAsyncTimeout || 1000};
...
angular.module('vlocTemplates', ['vlocity'])
  ...
  .config(['remoteActionsProvider', function(remoteActionsProvider) {
    ...
    }).service('vlocTemplateService', function () {
         ...
         function($scope, $rootScope, $element, $q, $controller, $attrs, timeStampInSeconds, actionService, dataService,
            ...
            var evalState = function(state) {
                ...
                if ($scope.obj) {
                    ...
                    angular.forEach(state.fields, function(field){
                        try {
                            var fieldLabel = $interpolate('{{'+(field.label.charAt(0) === '[' ? 'obj' + field.label : (field.label.indexOf(' ') > -1 ? '' :  field.label))+'}}')($scope);
             ...
             $scope.performAction = function(action, actionConfig) {
                ...
                if(action.isCustomAction) {
                    if (action.type === 'OmniScript') {
                        ...
                        action.url = '/apex/' + action.vForcewithNsPrefix + '?' + queryString;
                    }
                ...
                angular.forEach(extraParams, function(paramVal, paramKey){
                    if(checkForBracketField.test(paramVal)){
                        ...
                        extraParams[paramKey] = $interpolate('{{'+paramVal.replace('$scope.','') +'}}')($scope) === '' ? paramVal : $interpolate('{{'+paramVal.replace('$scope.','')+'}}')($scope);
                    }
    ...
    .provider('actionLauncher', function actionLauncherProvider() {
      ...
      this.$get = function($rootScope, dataService, actionService, $injector, $log, interactionTracking) {
        ...
        function openActionUrl(action, actionConfig) {
            ...
            toBeLaunchedUrl = urlHelper.pathname.replace(/(^\/?)/,'/') + urlHelper.search;
            ...
            return q.promise;
             ...
             return dataService.getCustomSettings($rootScope.nsPrefix + 'TriggerSetup__c').then(
                function(customSettings){
                    ...
                    // Setting cardpreview track by default to false    
                    $rootScope.vlocityCards.customSettings['Track.CardPreview'] = $rootScope.vlocityCards.customSettings['Track.CardPreview'] === undefined ? false : $rootScope.vlocityCards.customSettings['Track.CardPreview'];
                        ...
                        function(data) {
                            ...
                            if (data.records.length > 0) {
                                ...
                                if (cardNames.length > 0) {
                                    ...
                                    var cardsSoql = 'SELECT Id, Name, ' + nsPrefix + 'Type__c, ' + nsPrefix + 'Definition__c, LastModifiedDate, ' + nsPrefix + 'Version__c, ' + nsPrefix + 'GlobalKey__c,' + nsPrefix + 'Author__c,' + nsPrefix + 'Active__c FROM ' + nsPrefix + 'VlocityCard__c WHERE  ' + nsPrefix + 'Active__c = true AND Name IN ';
        ...
        getNamedCredentials: function() {
            return force.query('SELECT DeveloperName,Endpoint,Id,Language,MasterLabel,NamespacePrefix,PrincipalType FROM NamedCredential LIMIT 10000').then(
                function(data) {
                    ...
                    $log.debug('getNamedCredentials completed ',data.records, credentials);
                    return credentials;
